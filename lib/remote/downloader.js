...
const { printInfo, printWarning, OP_SKILLS_DIR, validateName, OPENPERSONA_DIRECTORY, OPENPERSONA_SKILLS_REGISTRY } = require('../utils');
...
async function downloadPrivate(target, outDir, options = {}) {
  ...
  if (isValidPackRoot(outDir)) {
    ...
    if (entries.length === 1) packRoot = path.join(outDir, entries[0]);
  }
  if (!packRoot) {
    throw new Error('Delivered archive is not a valid persona pack (no persona.json or SKILL.md found).');
  }

  // Record royalty provenance from the gated delivery so a later `openpersona
  // fork` can inherit the root creator (resale royalty). Store is authoritative.
  writePurchaseProvenance(packRoot, { delivery, target, orderId });
  return packRoot;
}

/**
 * Persist royalty lineage from a gated delivery into the installed pack's
 * `soul/lineage.json`. Merges over any lineage the pack already carries so the
 * root creator + canonical pack ref survive a later fork → resale, which is
 * what lets the Store pay royalty to the original author.
 *
 * Best-effort: provenance is non-fatal to a successful install.
 */
function writePurchaseProvenance(packRoot, { delivery, target, orderId }) {
  try {
    const soulDir = path.join(packRoot, 'soul');
    fs.ensureDirSync(soulDir);
    const lineagePath = path.join(soulDir, 'lineage.json');
    ...
    if (delivery && delivery.root_creator) lineage.rootCreator = delivery.root_creator;
    if (delivery && typeof delivery.generation === 'number') lineage.generation = delivery.generation;
    lineage.rootPackRef = (delivery && delivery.pack_ref) || target;
    lineage.purchasedVia = 'op-store-redeem';
    if (orderId) lineage.purchasedOrderId = orderId;
    // Gen-0 originals carry no explicit generation from Store — default to 0.
    if (lineage.generation === undefined || lineage.generation === null) lineage.generation = 0;

    fs.writeFileSync(lineagePath, JSON.stringify(lineage, null, 2));
  } catch (e) {
    printWarning(`Could not record purchase provenance: ${e.message}`);
  }
}
